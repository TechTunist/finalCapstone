...
class Shoe:
    ...
    # return cost of each Show instance
    def get_cost(self):
        return self.cost

    # return quantity of eacn Shoe instance
    def get_quantity(self):
        return self.quantity

    # return string representation of code attribute for each class instance
    def __str__(self):
...
# store a list of class objects
shoe_list = []

#==========Functions outside the class==============

# open a given text file in CSV format: "Country,Code,Product,Cost,Quantity"...
#... create a show object for each line in the txt file and add it to 'shoe_list'
def read_shoes_data(filename):
    
    # open file and save data to 'data_list'
    ...
    try:
        ...
        for i in range(1, len(data_list)):
            # tokenise each element in each line
            buffer = data_list[i].split(',')            

            # create new class instance from tokenised data
            new_shoe = Shoe(
                buffer[0],
                buffer[1],
                buffer[2],
        ...
        print("The was an error when reading the text file.")

# create new entry by user input
def capture_shoes(shoe_list):

    # get user input for creation of new shoe instance
    country = input("\nEnter the country: \n")
    code = input("\nEnter the code: \n")
    product = input("\nEnter the product: \n")

    # input validation - cost must be able to be cast to type int
    try:
        cost = input("\nEnter the cost: \n")
        quantity = input("\nEnter the quantity: \n")

        # attmept to convert to int to check acceptable input
        cost = int(cost)
        quantity = int(quantity)

    # print error message and early return
    except ValueError:
        print("\n+++++ ENTRY FAILED +++++\n\nYou must only enter numbers for 'cost' and 'quantity'.")
...
def highest_qty(shoe_list):
    ...
    print(f"\nThe {shoe_list[high_index].product} should be marked for sale!!\nWe have {high} units in stock.\n")


#==========Main Menu=============
print("\nWelcome to the warehouse helper. Choose an option from the menu below...\n")
